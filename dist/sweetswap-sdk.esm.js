    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (amountOut.token.equals(tokenOut) && currentPairs.length > 2) {
        ...
        if (Ea < Eb) {
          ...
          sortedInsert(bestTrades, new Trade(new Route([].concat(currentPairs, [pair]), originalAmountIn.currency, currencyOut), originalAmountIn, TradeType.EXACT_INPUT, getOptimalAmount(Ea, Eb), getAmountOut(getOptimalAmount(Ea, Eb), Ea, Eb)), maxNumResults, tradeComparator, tradeFilter);
        }
