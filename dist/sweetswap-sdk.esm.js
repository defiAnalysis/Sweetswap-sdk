...
var Trade = /*#__PURE__*/function () {
  ...
  currentPairs, originalAmountIn, bestTrades) {
    ...
    !originToken.token.equals(tokenOut) ? process.env.NODE_ENV !== "production" ? invariant(false) : invariant(false) : void 0;
  ...
  currentPairs, originalAmountIn, bestTrades) {
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (amountOut.token.equals(tokenOut)) {
        sortedInsert(bestTrades, new Trade(new Route([].concat(currentPairs, [pair]), originalAmountIn.currency, currencyOut), originalAmountIn, TradeType.EXACT_INPUT, ZERO, ZERO), maxNumResults, tradeComparator, tradeFilter);
      } else if (maxHops > 1 && pairs.length > 1) {
        var pairsExcludingThisPair = pairs.slice(0, i).concat(pairs.slice(i + 1, pairs.length)); // otherwise, consider all the other paths that lead from this token as long as we have not exceeded maxHops
  ...
  currentPairs, originalAmountOut, bestTrades) {
    ...
    for (var i = 0; i < pairs.length; i++) {
      ...
      if (amountIn.token.equals(tokenIn)) {
        sortedInsert(bestTrades, new Trade(new Route([pair].concat(currentPairs), currencyIn, originalAmountOut.currency), originalAmountOut, TradeType.EXACT_OUTPUT, ZERO, ZERO), maxNumResults, tradeComparator, tradeFilter);
      } else if (maxHops > 1 && pairs.length > 1) {
...
export { ChainId, Currency, CurrencyAmount, ETHER, Fetcher, Fraction, InsufficientInputAmountError, InsufficientReservesError, MINIMUM_LIQUIDITY, Pair, Percent, Price, Rounding, Route, Router, Token, TokenAmount, Trade, TradeType, WETH, arbComparator, currencyEquals, inputOutputComparator, tradeComparator, tradeFilter };
